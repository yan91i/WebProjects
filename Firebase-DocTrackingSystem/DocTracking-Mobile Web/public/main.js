    ...
    firebaseRef.child(num).once('value').then(function(snapshot) {
      ...
      for (var x in obj){
        ...
        if(checkName == currentUser){
          ...
          var lastUpdate=nowTime.toLocaleString('en-us',{ hour12: false });
    ...
    firebaseRef.child(num).once('value').then(function(snapshot) {
      ...
      for (var x in obj){
        ...
        if(checkName == currentUser){
          ...
          var lastUserNum = snapshot.child("UserNumber").val();
          if (lastUserNum == x){
            firebaseRef.child(num).child("Complete").set("Yes");
            window.alert("This circulation has been completed. Please return this document to Ava!");
          }
          toTask();
        }
      }
