  ...
  firebase.auth().onAuthStateChanged(function (user) {
    if (user) {
      ...
      firebaseRef.once('value').then(function (snapshot) {
        // Detect whether user is admin
        var adminList = snapshot.child("admin").val();
        for (var i in adminList) {
          var admin = snapshot.child("admin").child(i).val()
          if (uEmail == admin) {
...
function toProjectList() {
  ...
  document.getElementById("projectList").innerHTML =
    ...
    '<th>Project Name</th>' +
    '<th>Latest Hard Clash Number</th>' +
    '<th>Latest Clash Report Date</th>' +
    '</tr>' +
    '</thead>' +
    '<tbody id="pTable">' +
    '</tbody>' +
  ...
  firebaseRef.once('value').then(function (snapshot) {
    ...
    for (var y in admin) {
      var check = snapshot.child("admin").child(y).val()
      if (check == uEmail) {
        var stringValue = "true";
        var adminCheck = (/true/i).test(stringValue)
      }
    }
    ...
    if (adminCheck) {
      for (var x in obj) {
        if (x != "admin") {
          var projectNumber = x;
          var projectName = snapshot.child(x).child("pName").val();
          var projectHardClash = snapshot.child(x).child("latest").child("hard").val();
          var projectDate = snapshot.child(x).child("latest").child("date").val();
          // Create Table
          document.getElementById("pTable").innerHTML +=
            '<tr onclick="jumpProject(this)">' +
            '<td>' + projectNumber + '</td>' +
            '<td>' + projectName + '</td>' +
            '<td>' + projectHardClash + '</td>' +
            '<td>' + projectDate + '</td>' +
            '</tr>';
        }
      }
      ...
      for (var x in obj) {
        var pmrList = snapshot.child(x).child("PMR").val();
        for (var y in pmrList) {
          var pmr = snapshot.child(x).child("PMR").child(y).val();
          if (uEmail == pmr) {
            if (x != "admin") {
              var projectNumber = x;
              var projectName = snapshot.child(x).child("pName").val();
...
function jumpProject(project) {
  window.pNumber = project.childNodes[0].innerHTML;
  ...
  document.getElementById("pSummary").innerHTML =
    ...
    '<br>' +
    '<button onclick="toProjectList()" type="submit" class="btn btn-outline-info btn-sm btn-block"> <- Back </button>' +
    '<br>' +
    '<button onclick="signOut()" type="button" class="btn btn-outline-danger btn-sm btn-block">Sign Out</button>' +
    '<br>' +
    '</div>' +
  ...
  firebaseRef.once('value').then(function (snapshot) {
    ...
    var report = snapshot.child(pNumber).val();
    window.pName = snapshot.child(pNumber).child("pName").val();
    ...
    // 遍历全部report
    for (var x in report) {
      var rAll = snapshot.child(pNumber).child(x).child("all").child("total").val();
      var rHard = snapshot.child(pNumber).child(x).child("hard").child("total").val();
      var rDate = x;
      if (rAll != null) {
